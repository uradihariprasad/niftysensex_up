...
def data_fetch_loop():
    ...
    while THREAD_RUNNING:
        try:
            ...
            # Emit data to all connected clients — split into typed events
            # (mirrors legacy scanner_update / trades_update / alerts_update / index_update pattern)
            emit_all_updates()
...
# ============================================================
# Event Payload Slicing (legacy-style split events)
# ============================================================
# The original prototype emitted four narrow, purpose-built socket
# events instead of one large blob: scanner_update (overview/ranking
# data), index_update (full instrument/chart data), trades_update
# (trade suggestions only), and alerts_update (alerts only). We
# rebuild that same shape here, but keyed by index (NIFTY/SENSEX)
# instead of by individual stock symbol, since this app always
# tracks both indices together rather than a user-selected single
# stock. Nothing about the underlying analysis changes — these are
# just different views over serialize_market_data().

def build_scanner_payload():
    """Overview/ranking slice: ltp, change, dashboard summary, oi snapshot."""
    def overview(idx_name):
        full = serialize_market_data(idx_name)
        return {
            'ltp': full['ltp'],
            'open': full['open'],
            'high': full['high'],
            ...
            'oi_analysis': full['oi_analysis'],
            'market_breadth': full['market_breadth'],
...
def build_index_payload():
    """Full instrument/chart slice: candles, vwap, supports/resistances, SMC, option chain."""
    return {
...
def build_trades_payload():
    """Trade-suggestion-only slice."""
    return {
        'NIFTY': market_data['NIFTY'].get('trade_suggestion', {}),
        'SENSEX': market_data['SENSEX'].get('trade_suggestion', {}),
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }
...
def build_alerts_payload():
    """Alerts-only slice."""
    return {
        'NIFTY': market_data['NIFTY'].get('alerts', []),
        'SENSEX': market_data['SENSEX'].get('alerts', []),
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    }


def emit_all_updates():
    """Emit all four typed events to every connected client (background loop)."""
    try:
        socketio.emit('scanner_update', build_scanner_payload())
        socketio.emit('index_update', build_index_payload())
        socketio.emit('trades_update', build_trades_payload())
        socketio.emit('alerts_update', build_alerts_payload())
    except Exception as e:
...
def api_connect():
    """Legacy-style alias for /api/set_token, accepting {token} instead of {access_token}.
    Behaves identically — validates against Upstox and starts the background data thread."""
    ...
    try:
        ...
        if resp.status_code not in (200, 429):
            return jsonify({'status': 'error',
                             'message': f'Token invalid: HTTP {resp.status_code} — {resp.text[:200]}'}), 401
    except Exception as e:
    ...
    return jsonify({'status': 'success', 'message': 'Connected — live data starting'})
...
def api_scanner():
    """Overview/ranking slice (mirrors legacy scanner_update payload) for REST polling."""
    return jsonify(build_scanner_payload())


@app.route('/api/trades')
def api_trades():
    """Trade-suggestion slice (mirrors legacy trades_update payload) for REST polling."""
    return jsonify(build_trades_payload())


@app.route('/api/alerts')
def api_alerts():
    """Alerts slice (mirrors legacy alerts_update payload) for REST polling."""
    return jsonify(build_alerts_payload())


@app.route('/api/indices')
def api_indices():
    """Full instrument/chart slice (mirrors legacy index_update payload) for REST polling."""
    return jsonify(build_index_payload())
...
def handle_connect():
    ...
    if ACCESS_TOKEN and THREAD_RUNNING:
        # Send initial data — replay all four typed events, same as legacy app
        # replaying scanner_update/trades_update/alerts_update/index_update on connect
        emit('scanner_update', build_scanner_payload())
        emit('index_update', build_index_payload())
        emit('trades_update', build_trades_payload())
        emit('alerts_update', build_alerts_payload())
